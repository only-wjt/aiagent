...
AI Agent is a desktop application built with Tauri (Rust backend) and Vue 3 (TypeScript frontend). It provides a unified interface for managing Codex Agent sessions with support for multiple AI providers (Codex, OpenAI, Gemini) and MCP (Model Context Protocol) tools.
...
- **sidecar**: Node.js HTTP server (compiled to binary) that manages Codex Agent sessions
...
3. **Sidecar (Node.js/Bun)** → HTTP server that:
   - Manages individual Agent sessions using Codex Agent SDK
...
- **agent/client.ts**: AgentSession wrapper around Codex Agent SDK, handles streaming and tool execution
...
- **Tool Execution**: Codex Agent SDK handles tool calls; adapters may need custom tool implementations
